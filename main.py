...
def investment_strategy(house_price, Hm, Hb, Sm, Sb):
    ...
    # calculate profit in house and crypto in the years 2017 - 2037
    ...
    crypto_profit = np.round(float(Sm * 2037 + Sb - Startup_capital), 2)
    house_return = np.round(100 * (house_profit / Startup_capital), 2)  # percentage
    crypto_return = np.round(100 * (crypto_profit / Startup_capital), 2)  # percentage

    return house_profit, crypto_profit, house_return, crypto_return
...
def visualization(x_axes, Hy_axes, Sy_axes, Hm, Hb, Sm, Sb):
    ...
    plt.scatter(x_axes, Sy_axes, alpha=1, color='firebrick', label="BTC 1000 Cryptos Price")
    ...
    # add text
    plt.title("NYC Housing/BTC 1000 Cryptos Price over Time")
...
def report(Hp, Sp, Hr, Sr):
    ...
    # create text report
    plt.figtext(0.5, 0.5, "NYC Housing and BTC crypto prediction report:\n\n"
                          "Profit prediction for investment in housing in NYC\n"
                          "in 2017 after 20 years is: " + str(Hp) +
                "K ₪.\nThe return is: " + str(Hr) +
                "%.\n\n Profit prediction for investment in the BTC crypto\n"
...
def main():
    ...
    NYC = read_houses_data("nyc-rolling-sales.csv")
    data_array = houses_array_manipulation(NYC)
    X_year = data_array[0]  # predictor
    Y_house = data_array[1]  # response

    Y_spBTC = read_crypto_data("BTC_data.csv")
    ...
    Sx_train, Sx_test, Sy_train, Sy_test = train_test_split(
        X_year, Y_spBTC, test_size=0.33, random_state=42)
    ...
    Crypto_linreg = linear_model.LinearRegression()
    Crypto_linreg.fit(Sx_train.reshape(-1, 1), Sy_train)

    Hp, Sp, Hr, Sr = investment_strategy(Y_house[-1],
                                         Houses_linreg.coef_, Houses_linreg.intercept_,
                                         Crypto_linreg.coef_, Crypto_linreg.intercept_)
    # present in a graph
    fig1 = visualization(X_year, Y_house, Y_spBTC,
                           Houses_linreg.coef_, Houses_linreg.intercept_,
                           Crypto_linreg.coef_, Crypto_linreg.intercept_)
